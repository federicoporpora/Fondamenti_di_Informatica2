...
public class Parcometro {
	...
	public Ticket emettiTicket(LocalTime inizio, LocalTime fine) {
		...
		double costo;
		// minimo di durata: se durata effettiva � inferiore, si considera la durata minima, andrebbe fattorizzato il codice
		Duration durataSosta;
		if (fine.isBefore(inizio) || LocalTime.of(0, 0).equals(fine)) {
			durataSosta = Duration.between(inizioEffettivo, LocalTime.of(23, 59)).plusMinutes(1);
		} else {
			durataSosta = Duration.between(inizioEffettivo, fine);
		}
