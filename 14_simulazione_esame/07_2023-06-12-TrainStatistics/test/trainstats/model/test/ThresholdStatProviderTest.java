...
class ThresholdStatProviderTest {
	...
	@Test
	void testOK() {
		var provider = new ThresholdStatProvider(5);
		assertEquals(100, provider.getStatistics(train));
		assertTrue(provider.hasThreshold());
		assertEquals(5, provider.getThreshold());
		provider.setThreshold(10);
		assertEquals(10, provider.getThreshold());
		assertEquals(71, provider.getStatistics(train));
		provider.setThreshold(15);
		assertEquals(15, provider.getThreshold());
		assertEquals(14, provider.getStatistics(train));
	}
}
